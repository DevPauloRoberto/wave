                ...
                <div class="card bg-white p-4 md:p-6 rounded-xl shadow-sm flex justify-center">
                    ...
                    >
                        <div class="flex flex-col gap-2">
                            <label for="nome" class="font-bold text-gray-600">Nome da Categoria</label>
                            <InputText 
                                name="nome" 
                                type="text" 
                                placeholder="Ex: Noticia" 
                                fluid 
...
const { data: categoria, pending, error } = await useFetch<CategoriaItem>(`/api/admin/categorias/${categoriaId}`);
...
const onFormSubmit = async (event: any) => {
    ...
    if (valid) {
        saving.value = true;
        try {
            await $fetch(`/api/admin/categorias/${categoriaId}`, {
                method: 'PUT',
                body: {
                    nome: values.nome
                }
            ...
            toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Categoria atualizada com sucesso!', life: 3000 });
